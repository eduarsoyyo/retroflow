              ...
              <div className="rounded-card border-2 border-[#5856D6]/20 bg-[#5856D6]/3 dark:bg-[#5856D6]/5 p-4">
                ...
                <div className="mt-3 pt-3 border-t border-[#5856D6]/10 overflow-auto">
                  <table className="w-full text-[10px]">
                    ...
                    <tbody>
                      <tr><td className="py-1 px-1 font-semibold text-[#007AFF]">Venta</td>{MO.map((_, i) => { let r = monthlyRevenueFromServices(services, yr, i); if (simFrom || simTo) { const ms = new Date(yr,i,1); const me = new Date(yr,i+1,0); const sf = simFrom ? new Date(simFrom) : new Date(yr,0,1); const st = simTo ? new Date(simTo) : new Date(yr,11,31); if (me < sf || ms > st) r = 0 } return <td key={i} className="text-center px-0.5 text-[#007AFF]">{r > 0 ? fmt(r) : '\u2014'}</td> })}<td className="text-center px-1 font-bold text-[#007AFF]">{fmt(simTotRev)}</td></tr>
                      <tr><td className="py-1 px-1 font-semibold text-[#FF9500]">Coste</td>{MO.map((_, i) => { let mc = 0; monthlyData.forEach(p => { const si3 = getSalaryInfo(team.find(t=>t.id===p.id)!); const eSal = simSalaryOverrides[p.id] ?? si3.salary; const cH2 = ((eSal * si3.multiplier) / ((calendarios.find(c=>c.id===(team.find(t=>t.id===p.id) as unknown as Record<string,unknown>)?.calendario_id) as unknown as Record<string,unknown>)?.convenio_hours as number || 1800)); const oD = orgData.find(o=>o.member_id===p.id)?.dedication||0; const eD = simDedOverrides[p.id] !== undefined ? simDedOverrides[p.id]!/100 : oD; const h = oD > 0 ? p.months[i]!.hours * (eD/oD) : 0; mc += Math.round(h * cH2) }); return <td key={i} className="text-center px-0.5 text-[#FF9500]">{mc > 0 ? fmt(mc) : '\u2014'}</td> })}<td className="text-center px-1 font-bold text-[#FF9500]">{fmt(simTotCost)}</td></tr>
                      <tr className="border-t border-[#5856D6]/10"><td className="py-1 px-1 font-bold" style={{color: simMargin >= 0 ? '#34C759' : '#FF3B30'}}>Margen</td>{MO.map((_, i) => { let rv = monthlyRevenueFromServices(services, yr, i); if (simFrom || simTo) { const ms = new Date(yr,i,1); const me = new Date(yr,i+1,0); const sf = simFrom ? new Date(simFrom) : new Date(yr,0,1); const st = simTo ? new Date(simTo) : new Date(yr,11,31); if (me < sf || ms > st) rv = 0 } let mc2 = 0; monthlyData.forEach(p => { const si4 = getSalaryInfo(team.find(t=>t.id===p.id)!); const eSal2 = simSalaryOverrides[p.id] ?? si4.salary; const cH3 = ((eSal2 * si4.multiplier) / ((calendarios.find(c=>c.id===(team.find(t=>t.id===p.id) as unknown as Record<string,unknown>)?.calendario_id) as unknown as Record<string,unknown>)?.convenio_hours as number || 1800)); const oD2 = orgData.find(o=>o.member_id===p.id)?.dedication||0; const eD2 = simDedOverrides[p.id] !== undefined ? simDedOverrides[p.id]!/100 : oD2; const h2 = oD2 > 0 ? p.months[i]!.hours * (eD2/oD2) : 0; mc2 += Math.round(h2 * cH3) }); const mg = rv - mc2; return <td key={i} className="text-center px-0.5 font-bold" style={{color: mg >= 0 ? '#34C759' : '#FF3B30'}}>{rv > 0 || mc2 > 0 ? fmt(mg) : '\u2014'}</td> })}<td className="text-center px-1 font-bold" style={{color: simMargin >= 0 ? '#34C759' : '#FF3B30'}}>{fmt(simMargin)}</td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
