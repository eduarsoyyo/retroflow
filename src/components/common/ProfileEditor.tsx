// ═══ PROFILE EDITOR — Edit user avatar, color, name, contact ═══
...
export function ProfileEditor({ user, profile, onClose, onSave }: ProfileEditorProps) {
  ...
  const handleSave = async () => {
    ...
    if (result.ok) {
      onSave(result.data);
      // Update localStorage session so sidebar reflects changes
      try {
    ...
    setSaving(false);
    onClose();
    // Reload to propagate user changes to App state
    window.location.reload();
  };

  const labelStyle = { fontSize: 10, color: '#86868B', display: 'block', marginBottom: 3, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: 0.4 };
  const inputStyle = { width: '100%', border: '1.5px solid #E5E5EA', borderRadius: 10, padding: '8px 12px', fontSize: 13, fontFamily: 'inherit', outline: 'none', background: '#F9F9FB', boxSizing: 'border-box' as const };

  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,.4)', backdropFilter: 'blur(4px)' }} />
      <div style={{ position: 'relative', background: '#FFF', borderRadius: 20, width: '90%', maxWidth: 440, maxHeight: '85vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,.2)', padding: 24 }}>

        {/* Header with avatar */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, paddingBottom: 16, borderBottom: '1px solid #F2F2F7', marginBottom: 16 }}>
          <div onClick={() => setShowAvatars(!showAvatars)}
            style={{ width: 60, height: 60, borderRadius: 16, background: f.color || '#E5E5EA', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, cursor: 'pointer', position: 'relative' }}>
            {f.avatar || '👤'}
          </div>
          <div style={{ flex: 1 }}>
        ...
        {/* Avatar picker */}
        {showAvatars && (
          <div style={{ marginBottom: 14, padding: 12, background: '#F9F9FB', borderRadius: 12 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#86868B', textTransform: 'uppercase', marginBottom: 8 }}>Avatar</div>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {AVATARS.map(a => (
                <button key={a} onClick={() => { setF({ ...f, avatar: a }); setShowAvatars(false); }}
            ...
            <div style={{ fontSize: 10, fontWeight: 700, color: '#86868B', textTransform: 'uppercase', marginTop: 12, marginBottom: 8 }}>Color</div>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {COLORS.map(c => (
            ...
            <div style={{ fontSize: 10, fontWeight: 700, color: '#86868B', textTransform: 'uppercase', marginTop: 12, marginBottom: 8 }}>Casa</div>
